...
describe("AddBackendModal – connection chooser", () => {
  it("renders only the local Agent Server option", () => {
  ...
  it.skip("legacy Cloud authorization is hidden in Thingo Code", async () => {
  ...
  it("keeps the modal open when the local backend is below the compatible version floor", async () => {
    ...
    expect(await screen.findByTestId("add-backend-error")).toHaveTextContent(
      "Thingo Code requires agent-server 1.28.0 or newer",
    );
    expect(onClose).not.toHaveBeenCalled();
  });
  ...
  it.skip("legacy Cloud advanced settings are hidden in Thingo Code", async () => {
    const user = userEvent.setup();
    renderWithProviders(<AddBackendModal onClose={vi.fn()} />);
  ...
  it.skip("legacy Cloud login is hidden in Thingo Code", () => {
    renderWithProviders(<AddBackendModal onClose={vi.fn()} />);
