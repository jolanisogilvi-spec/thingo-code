...
describe("committed desktop icon artifacts", () => {
  it("has no low-alpha colour speckles that glow after Windows downscaling", () => {
    expect(normalizeIconPng(committedPng).equals(committedPng)).toBe(true);
  });
...
describe("electron-builder icon wiring", () => {
  it("points buildResources at the directory holding the committed icons", () => {
    ...
    expect(config.directories?.buildResources).toBe("electron/build-resources");
  });
