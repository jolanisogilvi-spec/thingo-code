...
describe("ManageBackendsModal", () => {
  ...
  it("shows disconnected for a reachable backend below the compatible version floor", async () => {
    ...
    ).toHaveTextContent("Thingo Code requires agent-server 1.28.0 or newer");
  });
  ...
  it.skip("retired: Cloud reconnect recovery controls are not available in Thingo Code", async () => {
  ...
  it.skip("retired: Cloud reconnect cannot update a token in Thingo Code", async () => {
    vi.stubEnv("VITE_LOCK_TO_CLOUD", "https://app.all-hands.dev");
  ...
  it.skip("retired: Cloud reconnect authorization is not available in Thingo Code", async () => {
    vi.stubEnv("VITE_LOCK_TO_CLOUD", "https://app.all-hands.dev");
  ...
  it("hides the Cloud reconnect controls when the locked backend uses cookie auth", async () => {
    ...
    expect(screen.getByTestId("manage-backends-add")).toBeInTheDocument();
  });
...
describe("BackendRow", () => {
  ...
  it("shows a logged-out cloud status without a login button", () => {
    const onLogin = vi.fn();
  ...
  it.skip("retired: Cloud device authorization is hidden in Thingo Code", async () => {
    const user = userEvent.setup();
    const popup = {
      closed: false,
      close: vi.fn(),
  ...
  it("ignores the legacy VITE_LOCK_TO_CLOUD mutation lock", () => {
    ...
    const row = screen.getByTestId(`manage-backends-row-${cloudBackend.name}`);
    expect(
      within(row).getByTestId(`manage-backends-edit-${cloudBackend.name}`),
    ).toBeInTheDocument();
    expect(
      within(row).getByTestId(`manage-backends-remove-${cloudBackend.name}`),
    ).toBeInTheDocument();
  });

  it("ignores the legacy runtime Cloud mutation lock", () => {
    (
