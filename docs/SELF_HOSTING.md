# Self-Hosting Thingo Code on a Virtual Machine

This guide walks through running Thingo Code on a virtual machine (VM) so you
can reach it from anywhere via a browser.

> [!WARNING]
> Thingo Code drives an agent that can read and write the filesystem of the
...
3. **Run Thingo Code** — generate a key with `openssl rand -base64 32`, then `export LOCAL_BACKEND_API_KEY=<key>` and `npx @openhands/agent-canvas --public`
4. **(Optional) Get a domain** — point a domain at the machine with nginx + Let's Encrypt for TLS
5. **(Optional) Connect locally** — add the remote as a backend in your local Thingo Code
...
Start Thingo Code in public mode:
...
`LOCAL_BACKEND_API_KEY`, and confirm that you land in Thingo Code.

## 5. (Optional) Connect your local Thingo Code to the remote machine

If you already run Thingo Code locally, you can register the remote machine
as an additional backend and switch between local and remote from the UI.

1. In your local Thingo Code, open **Manage backends** → **Add a backend**:
