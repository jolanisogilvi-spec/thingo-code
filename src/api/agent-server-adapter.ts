...
import thingoCodeIdentityPrompt from "#/prompts/thingo-code-identity.md?raw";
...
export function buildRuntimeServicesSystemSuffix(
  ...
  if (automation?.url_from_agent) {
    lines.push(
      ...
      `    ${automation.description ?? "Thingo Code Automations service."}`,
    );
...
function buildConfiguredOpenHandsAgentSettings(
  ...
  return {
    ...agentSettings,
    // Keep the backend protocol value for compatibility while exposing only
    // the Thingo Code product identity in the UI.
...
function buildConfiguredAgentSettings(
  ...
  return buildConfiguredOpenHandsAgentSettings(
    settings,
    runtimeServicesInfo,
    query,
  );
...
export function buildStartConversationRequest(
  ...
  const payload: AgentSettingsStartConversationPayload = {
    // Thingo Code always launches through inline agent settings so the product
    // identity prompt and client-owned runtime context cannot be bypassed by a
    // named Agent Profile.
  ...
  // Thingo Code always launches the native inline agent, whose encrypted LLM
  // and MCP settings must be decrypted by the agent-server.
