...
| Thingo Code / Agent Server | DefenseClaw equivalent |
|---|---|
...
| Component | Version |
|---|---|
| Thingo Code / Agent Server | Current `main` |
| Python | 3.10+ |
...
Follow the standard [Thingo Code quickstart](../README.md). The integration steps below assume the Agent Server is reachable at `http://localhost:18000`.
...
**Configure the LLM base URL in Thingo Code:**

Open the Thingo Code settings panel → select your active backend → under **LLM settings**, set **Base URL** to:
...
**Workflow recommendation:** Add `defenseclaw skill scan <skill-dir>` as a pre-commit or CI step in repositories that ship skills for Thingo Code.
...
The following integrations would require changes to Thingo Code, the Agent Server, or DefenseClaw, but would significantly deepen the security posture.
...
### 4. Thingo Code security dashboard

A dedicated panel in the Thingo Code UI that queries DefenseClaw's gateway REST API (`GET /alerts`, `GET /enforce/blocked`) would surface guardrail findings inline with the conversation view — correlating blocked prompts or tool calls with the agent turn that triggered them.
...
- [Thingo Code Self-Hosting](./SELF_HOSTING.md)

---

_This document is maintained as part of the Thingo Code documentation._
