...
const CANVAS_UI_DESCRIPTION = `The user is interacting with you inside Thingo Code — a web UI with a chat panel on the left and a tabbed right-side panel (files, terminal, browser, vscode, planner, tasklist). This tool lets you drive that right-side panel so the user sees what you just produced.
...
* You browsed to a URL the user should see →
    ...
    browser interaction so Thingo Code has a screenshot to display, then call
