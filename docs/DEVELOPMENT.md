...
### Prewarming Python dependencies

If the network is slow, download the pinned Python 3.12 runtime plus the Agent
Server and automation dependency trees before starting the stack:
...
The managed runtime is stored under `resources/python/`; its platform-specific
cache is stored under `resources/uv-cache/`. Both paths are ignored by Git.
The desktop release command runs this step automatically and includes both in
the installer. The packaged app always selects that Python runtime, rather than
the user's system Python, so a newly installed Thingo Code desktop app does
not need to fetch packages or fail because only a different CPython ABI is
available locally. The build also verifies both backend dependency trees in
offline mode before packaging.

## Repository boundaries

This repository contains the Thingo Code frontend and local-stack orchestration. Use the sibling repositories for their owned layers:
...
| `OH_AUTOMATION_GIT_REF`   | Git ref for automation backend (overrides the pinned default version) | _(unset)_ |
| `OH_AGENT_SERVER_GIT_REF` | Git ref for agent-server (overrides the pinned default version)       | _(unset)_ |
