...
> Legacy compatibility reference: Thingo Code no longer exposes ACP Agent
> selection or starts new ACP conversations. This document is retained only so
> existing installations can understand and recover previously stored data.

The upstream compatibility layer can drive conversations with the built-in agent or
...
Thingo Code calling an LLM directly, the Agent Server spawns the agent's own CLI
as a subprocess and relays each turn to it. The external agent manages its own
LLM, tools, and execution; Thingo Code sends messages and renders what comes
back.

```mermaid
flowchart LR
    canvas["Thingo Code<br/>(this UI)"]
    server["Agent Server"]
...
The Agent Server owns the subprocess and the credentials; Thingo Code only
...
1. **Choose agent** — pick Claude Code, Codex, or Gemini CLI instead of
   the native agent. The choice is saved immediately to your backend's settings.
2. **Check backend** — confirms Thingo Code can reach the Agent Server.
...
- **Agent (legacy builds only)** — switch between the native agent and **ACP**.
