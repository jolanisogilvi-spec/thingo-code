...
const projectUvCacheDir = join(projectRoot, "resources", "uv-cache");
...
function showHelp() {
  console.log(`
Thingo Code + Automation Development Stack
...
function spawnService(name, command, args, options = {}) {
  const proc = spawn(
    ...
    getProcessTreeSpawnOptions({
      ...
      env: {
        ...process.env,
        ...(existsSync(projectUvCacheDir)
          ? { UV_CACHE_DIR: process.env.UV_CACHE_DIR || projectUvCacheDir }
          : {}),
        // Bound each stalled package request. The outer readiness deadline
        // remains authoritative, while this prevents one dead connection
        // from leaving a first launch apparently downloading forever.
        UV_HTTP_TIMEOUT: process.env.UV_HTTP_TIMEOUT || "30",
        ...options.env,
      },
      cwd: options.cwd,
    }),
...
async function waitForService(name, url, timeoutMs = 30000) {
  ...
  let lastError = null;
  const tracksSpawnedProcess = processes.has(name);

  while (Date.now() - start < timeoutMs) {
    if (tracksSpawnedProcess && !processes.has(name)) {
      logService(name, "Exited before becoming ready", c.red);
      return false;
    }
    try {
  ...
  const stalledProcess = processes.get(name);
  if (stalledProcess && isProcessRunning(stalledProcess)) {
    logService(name, "Stopping the timed-out startup process", c.yellow);
    signalProcessTree(stalledProcess, "SIGTERM");
  }
  return false;
}
...
function printBanner(config) {
  const stackName = config.frontendOnly
    ? "Thingo Code Frontend Stack"
    : config.backendOnly
      ? "Thingo Code Backend Stack"
      : "Thingo Code + Automation Stack";
...
async function main(options = {}) {
  const {
    bannerTitle = "Thingo Code + Automation Development Stack",
