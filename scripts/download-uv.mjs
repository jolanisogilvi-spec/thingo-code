...
async function resolveVersion() {
  ...
  try {
    ...
    console.warn(
      `[download-uv] GitHub API unavailable (${error.message}); ` +
        "falling back to the public release redirect.",
    );
  }

  // The release page redirects to /releases/tag/<version> and does not
  // require an API token, so it continues to work when shared CI/build IPs
  // exhaust GitHub's unauthenticated API quota.
  return fetchLatestReleaseVersion();
}
...
function fetchLatestReleaseVersion() {
  return new Promise((resolve, reject) => {
    get(
      ...
      { headers: { "User-Agent": "agent-canvas-build" } },
      (res) => {
        if (res.statusCode === 301 || res.statusCode === 302) {
        ...
        reject(
          new Error(
            `Could not parse latest uv release redirect (HTTP ${res.statusCode})`,
          ),
        );
      },
    ).on("error", reject);
...
async function main() {
  ...
  console.log(
    `[download-uv] Downloading uv v${version} for ${PLATFORM}/${ARCH}`,
  );

  const filename = `${spec.target}.${spec.ext}`;
  const url = `https://releases.astral.sh/github/uv/releases/download/${version}/${filename}`;
