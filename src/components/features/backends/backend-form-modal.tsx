 ...
 */
function inferKindFromHost(_host: string): BackendKind {
  return "local";
}
...
/**
 * Local Agent Server connection used by onboarding and backend settings.
 */
...
function AddBackendChooser({
  ...
  const [selectedOption, setSelectedOption] =
    React.useState<AddBackendOption>("agent-server");
  ...
  return (
    <div data-testid="add-backend-chooser" className="flex flex-col">
      <div
        ...
        className="grid grid-cols-1 overflow-hidden rounded-lg border border-[var(--oh-border)]"
      >
        <BackendOptionTab
          value="agent-server"
