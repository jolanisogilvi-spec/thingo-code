...
describe("buildStartConversationRequest", () => {
  ...
  it("normalizes legacy ACP settings to the Thingo Code native agent", () => {
    const agentSettings = {
      agent_kind: "acp",
      acp_server: "codex",
    ...
    expect(payload.agent_settings!.agent_kind).toBe("openhands");
    expect(payload.agent_settings).not.toHaveProperty("acp_server");
    expect(payload.agent_settings).not.toHaveProperty("acp_command");
    expect(payload.agent_settings!.mcp_config).toEqual(
      agentSettings.mcp_config,
    );
    expect(payload.secrets_encrypted).toBe(true);
  ...
  it("builds a raw planning agent request for local Planner", () => {
    ...
    const suffix = String(
      (payload.agent.agent_context as { system_message_suffix?: string })
        ?.system_message_suffix,
    );
    expect(suffix).toContain("Thingo Code");
    expect(suffix).toContain("Planning Agent");
  ...
  it("decrypts normalized legacy settings as a native-agent request", () => {
    const agentSettings = {
      agent_kind: "acp",
      acp_server: "codex",
      acp_command: ["codex-acp"],
...
describe("buildStartConversationRequest — agentProfileId path", () => {
  it("ignores agent_profile_id and sends identity-enriched inline settings", () => {
    const settings = makeSettings({
      agent_kind: "openhands",
      llm: { model: "litellm_proxy/openai/gpt-5.5", api_key: "sk-test" },
    });
    ...
    expect(payload.agent_settings?.agent_kind).toBe("openhands");
    expect(
      String(payload.agent_settings?.agent_context?.system_message_suffix),
    ).toContain("Thingo Code");
  ...
  it("keeps secrets_encrypted for the inline fallback", () => {
    const agentSettings = {
      agent_kind: "openhands",
      llm: {
        model: "litellm_proxy/openai/gpt-5.5",
