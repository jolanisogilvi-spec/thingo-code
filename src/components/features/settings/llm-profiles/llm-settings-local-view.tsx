...
/**
 * Basic mode normally clears a base URL after the model changes because that
 * URL may belong to the previous provider. The Thingo preset changes its
 * OpenAI-compatible model and endpoint as an intentional pair, so retain it.
 */
export const shouldDropBaseUrlForBasicProfileModelChange = (
  didChangeModelInBasic: boolean,
  ...
  const handleSave = useCallback(async () => {
    ...
    if (authType === LLM_AUTH_TYPE_SUBSCRIPTION) {
      ...
      if (isCloudOpenHandsProvider) {
        ...
        if (
          shouldDropBaseUrlForBasicProfileModelChange(
            didChangeModelInBasic,
            llmConfig,
          )
        ) {
      ...
      <LlmSettingsScreen
        ...
        initialValueOverrides={
          viewMode === "edit" && editingProfile?.initialValues
            ...
            : // Create mode: prefill the Thingo default. The model and endpoint
              // are dirty so a new profile can be validated and saved directly;
              // the API key remains blank until the user provides one.
              {
                ...THINGO_LLM_DEFAULT_VALUES,
                "llm.api_key": "",
                [LLM_PROVIDER_CONNECTION_KEY]: "",
        ...
        markInitialOverridesDirty={viewMode === "create"}
