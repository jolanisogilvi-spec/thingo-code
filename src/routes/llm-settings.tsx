...
interface ThingoPresetCardProps {
  model: string;
  baseUrl: string;
  isDisabled: boolean;
  onSelect: () => void;
...
function ThingoPresetCard({
  model,
  baseUrl,
  isDisabled,
  onSelect,
}: ThingoPresetCardProps) {
  const { t } = useTranslation("openhands");
  const isSelected = isThingoLlmConfiguration(model, baseUrl);

  return (
    <section
      data-testid="thingo-llm-preset"
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex flex-col gap-1">
          <h3 className="text-sm font-medium text-[var(--oh-foreground)]">
            {t(I18nKey.SETTINGS$THINGO_PRESET_TITLE)}
          </h3>
          <p className="text-xs leading-5 text-[var(--oh-muted)]">
            {t(I18nKey.SETTINGS$THINGO_PRESET_DESCRIPTION, {
              endpoint: THINGO_LLM_BASE_URL,
            })}
          </p>
        </div>
        <BrandButton
          testId="thingo-llm-preset-select"
          type="button"
        ...
        >
          {isSelected
            ? t(I18nKey.SETTINGS$THINGO_PRESET_ACTIVE)
            : t(I18nKey.SETTINGS$THINGO_PRESET_ACTION)}
        </BrandButton>
      </div>
    </section>
  );
...
/**
 * Basic LLM saves normally discard a provider-specific endpoint after a model
 * change. Thingo's preset needs both values, so keep its endpoint together
 * with the matching model.
 */
export const shouldPreserveThingoBaseUrlInBasicSave = (
  model: string,
  baseUrl: string,
) => isThingoLlmConfiguration(model, baseUrl);
...
export function LlmSettingsScreen({
  ...
  const buildHeader = React.useCallback(
    ({ values, isDisabled, view, onChange }: SdkSectionHeaderProps) => {
      ...
      const selectThingoPreset = () => {
        setEnableSubscriptionModels(false);
        onChange(LLM_AUTH_TYPE_KEY, LLM_AUTH_TYPE_API_KEY);
        onChange(LLM_SUBSCRIPTION_VENDOR_KEY, "");
        onChange(LLM_PROVIDER_CONNECTION_KEY, "");
        onChange("llm.model", THINGO_DEFAULT_LLM_MODEL);
        onChange("llm.base_url", THINGO_LLM_BASE_URL);
      };
  ...
  const buildPayload = React.useCallback(
    (
      ...
      if (authType === LLM_AUTH_TYPE_SUBSCRIPTION) {
        ...
        const selectedModel = String(context.values["llm.model"] ?? "");
