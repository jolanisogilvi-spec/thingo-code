...
describe("ChatInputActions", () => {
  ...
  it("hides the Change Agent button on a cloud backend", () => {
    setRegisteredBackends([cloudBackend]);
    setActiveSelection({ backendId: cloudBackend.id });
...
describe("ChatInputActions — Switch agent profile gate (OSS-5735)", () => {
  ...
  it("does not offer Switch agent profile on the home page", async () => {
    renderWithProviders(<ChatInputActions disabled={false} />, {
      navigation: { conversationId: null },
    });
  ...
  it("does not offer Switch agent profile in a blank conversation", async () => {
    useActiveConversationMock.mockReturnValue({
