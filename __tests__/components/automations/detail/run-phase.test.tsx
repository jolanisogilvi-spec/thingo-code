...
// Simplified Chinese ("zh-CN"), the product default language.
...
function translate(key: string, options?: Record<string, unknown>): string {
  const value = TRANSLATIONS[key]?.["zh-CN"] ?? key;
  if (!options) return value;
  // Interpolate the same way i18next does.
...
describe("RunPhase — known code, non-English language", () => {
  it("shows the Chinese translation.json value for a known phase code, not the raw code", () => {
    render(
      <RunPhase status={RUNNING} code="sandbox_provisioning" label={null} />,
    );

    const expected =
      TRANSLATIONS[I18nKey.AUTOMATIONS$DETAIL$PHASE_SANDBOX_PROVISIONING][
        "zh-CN"
      ];
...
describe("resolveRunPhaseText — one answer for the row and its tooltip", () => {
  ...
  const t = (key: string) => TRANSLATIONS[key]?.["zh-CN"] ?? key;
...
describe("formatRunPhaseAge — telling a moving run from a stalled one", () => {
  it("reports how long the run has held the phase, localized", () => {
    expect(formatRunPhaseAge(minutesAgo(12), "zh-CN", translate)).toBe(
      "12分钟前",
    );
  });

  it("reports a phase written seconds ago as 'just now', not as zero minutes", () => {
    expect(formatRunPhaseAge(minutesAgo(0), "zh-CN", translate)).toBe(
      TRANSLATIONS[I18nKey.AUTOMATIONS$DETAIL$TIME_JUST_NOW]["zh-CN"],
    );
  });

  it("boundary: 60 minutes rolls over to the hours wording", () => {
    expect(formatRunPhaseAge(minutesAgo(60), "zh-CN", translate)).toBe(
      "1小时前",
    );
  });

  it("returns null when the service reported no timestamp", () => {
    expect(formatRunPhaseAge(null, "zh-CN", translate)).toBeNull();
    expect(formatRunPhaseAge(undefined, "zh-CN", translate)).toBeNull();
  });
  ...
  it("negative: returns null for timestamps the formatter would print as garbage", () => {
    ...
    expect(formatRelativeTime("not-a-date", "zh-CN", translate)).toMatch(
      /Invalid/,
    );
    expect(
      formatRelativeTime("1970-01-01T00:00:00Z", "zh-CN", translate),
    ).toMatch(/1970/);

    expect(formatRunPhaseAge("not-a-date", "zh-CN", translate)).toBeNull();
    expect(
      formatRunPhaseAge("1970-01-01T00:00:00Z", "zh-CN", translate),
    ).toBeNull();
  });
});
...
describe("RunPhase — how long the run has been in this phase", () => {
  it("shows the age beside the phase, so a stalled run reads differently from a moving one", () => {
    ...
    expect(screen.getByTestId("run-phase-age")).toHaveTextContent("41分钟前");
  });
...
describe("RunPhase — reachable without a mouse", () => {
  it("leaves the whole phase, age included, in the accessibility tree", () => {
    ...
    expect(age).toHaveTextContent("7分钟前");
