/**
 * Electron Main Process — Thingo Code Desktop
 *
 * Starts the full Thingo Code stack (agent-server + automation via uvx,
...
// Thingo Code app icon, used as the BrowserWindow.icon option.
...
/**
 * Point uv at the cache generated by the desktop release build. The cache is
 * complete for the pinned Agent Server and automation versions, so packaged
 * launches can run offline and do not perform a first-launch PyPI download.
 */
function injectBundledUvCache() {
  if (!app.isPackaged) return;
  const cacheDir = join(process.resourcesPath, "uv-cache");
  const pythonDir = join(process.resourcesPath, "python");
  if (!existsSync(cacheDir) || !existsSync(pythonDir)) {
    console.warn(
      "[desktop] Offline Python bundle is incomplete; allowing uv to recover online.",
      { cacheDir, pythonDir },
    );
    return;
  }
  ...
  const pythonExecutable = installName
  ...
  if (!pythonExecutable || !existsSync(pythonExecutable)) {
    console.warn(
      `[desktop] Bundled Python ${pythonVersion} executable was not found; allowing uv to recover online.`,
      pythonDir,
    );
    return;
  }
  ...
  process.env.UV_PYTHON = pythonExecutable;
  process.env.UV_PYTHON_DOWNLOADS = "never";
  process.env.UV_PYTHON_INSTALL_DIR = pythonDir;
  process.env.UV_OFFLINE = "1";
  console.log(
    `[desktop] Using bundled Python ${pythonVersion} and prewarmed uv cache`,
    { pythonExecutable, cacheDir },
  );
}
...
function createMainWindow() {
  ...
  mainWin.webContents.setWindowOpenHandler(({ url }) => {
    // The legacy cloud device-flow opens about:blank immediately
  ...
  if (result?.agentServerReady === false) {
    throw new Error(
      "The agent server did not finish starting in time. " +
        ...
        "Python and the compatible agent-server from PyPI. " +
...
app.whenReady().then(async () => {
  ...
  // Thingo Code logo instead of the default Electron logo. In a packaged
  ...
  if (!uvxAvailable()) {
    dialog.showErrorBox(
      ...
      app.isPackaged
        ? "The bundled uv binary could not be found. Please reinstall Thingo Code."
  ...
  try {
    ...
    setBootPhase("正在连接 Agent Server…");
    ...
    const errorTail = recentServiceErrors.length
      ...
      : "";
    dialog.showErrorBox("Thingo Code failed to start", summary + errorTail);
    app.quit();
  }
});
