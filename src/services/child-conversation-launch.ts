...
async function launchCloudChild(
  ...
  if (!backend) {
    return failure(
      "Thingo Code supports local child conversations only.",
      'Relaunch with target="local".',
    );
  }
  ...
  return {
    ...
    parent_link: false,
    parent_link_note:
      "This conversation runs on the local agent server, so a remote backend has no parent to link the child to.",
  };
}
