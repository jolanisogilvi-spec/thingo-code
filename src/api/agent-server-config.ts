...
export function getLockedCloudHost(): string | null {
  // Legacy deployments may still inject VITE_LOCK_TO_CLOUD or the matching
  // window global. Thingo Code deliberately ignores both: new sessions are
  // local-only and the Cloud login surface must remain unavailable.
  return null;
}
