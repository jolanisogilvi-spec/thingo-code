 ...
 *
 * The bundled uv binary (resources/bin/) and managed Python runtime
 * (resources/python/) land in <Resources>/ via extraResources so Electron
 * can use the same Python minor version as the prewarmed wheel cache.
 *
...
const config = {
  ...
  //   python/ — managed CPython pinned in config/defaults.json and installed
  //             while prewarming the uv cache
  ...
  extraResources: [
    ...
    ...(existsSync(join(repoRoot, "resources", "python"))
      ? [{ from: "resources/python/", to: "python/", filter: ["**/*"] }]
      : []),
    ...(existsSync(join(repoRoot, "resources", "uv-cache"))
      ? [{ from: "resources/uv-cache/", to: "uv-cache/", filter: ["**/*"] }]
      : []),
  ],
  ...
  dmg: {
    ...
    artifactName: "Thingo-Code-${version}-${arch}.${ext}",
  },
  ...
  nsis: {
    ...
    artifactName: "Thingo-Code-Setup-${version}.${ext}",
  },
