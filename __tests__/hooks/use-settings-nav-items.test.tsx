...
describe("useSettingsNavItems", () => {
  ...
  it("shows a single Thingo Code Agent item on both local and cloud", () => {
    useConfigMock.mockReturnValue({ data: createConfig() });
    ...
    for (const kind of ["local", "cloud"] as const) {
      ...
      expect(paths.filter((p) => p === "/settings/agent")).toHaveLength(1);
    }
  });
  ...
  it("keeps Thingo Code Agent settings available when a legacy Cloud lock is configured", () => {
    // Legacy Cloud lock configuration is deliberately ignored by Thingo Code,
    // whose product flow is local-only and always exposes its single Agent page.
