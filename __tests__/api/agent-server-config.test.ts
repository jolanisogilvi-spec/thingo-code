...
describe("getLockedCloudHost", () => {
  ...
  it("ignores the legacy VITE_LOCK_TO_CLOUD setting", () => {
  ...
  it("ignores the legacy runtime-injected Cloud URL", () => {
    setInjectedCloudHost("https://runtime.example.com/");

    expect(getLockedCloudHost()).toBeNull();
  });

  it("does not activate a Cloud lock for hostnames without a scheme", () => {
...
describe.skip("retired getLockedCloudAuthMode compatibility behavior", () => {
