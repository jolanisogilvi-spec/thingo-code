...
describe("backend-registry storage", () => {
  ...
  it("returns empty list when storage is malformed", () => {
    ...
    expect(readStoredBackends()).toEqual([
      expect.objectContaining({ id: "default-local", kind: "local" }),
    ]);
  });
  ...
  it("ignores a legacy Cloud lock and preserves stored local backends", () => {
    (
  ...
  it("does not seed Cloud for a legacy transition-domain lock", () => {
  ...
  it("does not alter legacy Cloud data when Cloud mode is disabled", () => {
    (
      window as unknown as Record<string, unknown>
    ).__AGENT_CANVAS_LOCK_TO_CLOUD__ = window.location.origin;
